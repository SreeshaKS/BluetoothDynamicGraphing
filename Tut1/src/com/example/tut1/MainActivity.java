...
public class MainActivity extends Activity implements View.OnClickListener{
	...
	Handler mHandler = new Handler(){
		...
		public void handleMessage(Message msg) {
			...
			case Bluetooth.MESSAGE_READ:
				...
				//String strIncom = new String(readBuf, 0, 2);
				// create string from bytes array
                String strIncom = ""+((double)((byte)readBuf[0]));
                ...
                if(true){
                    ...
                    if(true){
                        if(((double)((byte)readBuf[0]))<0) {
                            finalVal = -((double) ((byte) readBuf[0]));
                        }
                        else{
                            finalVal=-((double)((byte)readBuf[0]));
                        }
                   // if(true){
						Series.appendData(new GraphViewData(graph2LastXValue,finalVal),AutoScrollX);
                        for(int i=0;i<999;i++) {
                            for(int j=0;j<999;j++){
                        }
                        }
	...
	void init(){
		...
		Series = new GraphViewSeries("Signal", 
				new GraphViewStyle(Color.BLACK, 2),//color and thickness of the line
		...
		graphView.setManualYAxisBounds(255, -255);
	...
	public void onClick(View v) {
		...
		case R.id.tbStream:
			if (tbStream.isChecked()){
                ...
                Bluetooth.connectedThread.write("F");
			}else{
				if (Bluetooth.connectedThread != null)
